...
| `SKYFORGE_GLOBALPOSE_TOPICS` | _(空)_ | 多轨迹输入，逗号分隔；第一项为黑色主轨迹，其余自动分配颜色 |
...
| `SKYFORGE_SIMULATION_RATE_HZ` | `5` | 圆形模拟轨迹的输出频率 |
| `SKYFORGE_SIMULATION_PERIOD_SECONDS` | `24` | 圆形模拟轨迹一圈所需秒数 |
| `SKYFORGE_SIMULATION_RADIUS_RATIO` | `0.32` | 圆半径占地图较短边的比例 |
...
此模式使用 Python Gateway，不连接 ROS。若已加载 DOM/DSM，模拟器会在地图范围内通过 `/self_state/globalpose` 生成圆形轨迹，并保留完整 GeoTIFF 功能。
...
会以已加载 TIFF 的中心为圆心生成模拟定位轨迹和状态，用于集成测试。
...
| 话题 | 类型 | 频率 | 用途 |
|------|------|------|------|
| `SKYFORGE_GLOBALPOSE_TOPICS` 中的每个话题 | 定位程序实际的 `*/GlobalPose` | 1–20 Hz | 按话题分别使用 latitude/longitude 生成独立轨迹 |
...
```bash
SKYFORGE_GLOBALPOSE_TOPICS=/self_state/globalpose,/vehicle_2/globalpose npm run start:ros
```

不同话题的历史点不会互相连接。第一项沿用黑色 `50px` 主轨迹样式，其余话题使用不同实色，并在 DOM/DSM 地图左上角显示话题图例。

### 发布（输出）

| 话题 | 类型 | 触发 | 用途 |
|------|------|------|------|
| `/selected_region` 或自定义区域话题 | `skyforge_msgs/RegionCommand` | 默认 50 Hz | 向下游节点并发发送区域指令 |

区域发布支持多任务并发，默认以 `50 Hz` 持续发送。每次选择区域、标识位和话题后点击“添加发布”，都会创建一条独立任务；可以对单条任务停止，也可以全部停止。同一个话题允许交错接收多个区域，不同话题也可以同时发布。
...
区域订阅端可以同时订阅自车 `GlobalPose`，使用其中的 `latitude`、`longitude` 与 `RegionCommand.points` 判断：

- `rectangle`：取四个顶点经纬度的最小值和最大值，检查自车经纬度是否落在范围内。
- `polygon`：按消息中的顶点顺序执行 point-in-polygon（射线法或 winding number）。边界点建议按“区域内”处理并保留少量容差。
- 小范围地图可直接以 longitude 为 X、latitude 为 Y 判断；范围较大或要求高精度时，先统一转换到 TIFF 对应投影、UTM 或局部 ENU 坐标。

当前消息发布的是区域几何，不直接发布 `inside` 布尔值，因此接收端需要同时持有最新自车位置。矩形和多边形都具备完成判断所需的数据。
...
| POST | `/api/publish/start` | 添加或更新一条持续区域发布任务 |
| POST | `/api/publish/stop` | 传 `publicationId` 停止单条；空对象停止全部 |
...
| `topic` | `{name}` | 订阅话题名称变更 |
| `publish` | `{publicationId, topic, flag, region}` | 某条任务完成一次区域发布 |
| `publish-state` | `{publications: [...]}` | 全部并发发布任务状态变更 |
